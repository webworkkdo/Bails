...
const makeMessagesRecvSocket = (config) => {
    ...
    const callSpamDB = new node_cache_1.default({
        stdTTL: 0.5, // 0.5 second
        useClones: false
    });
    const groupNotificationSpamDB = new node_cache_1.default({
        stdTTL: 1, // 1 second
        useClones: false
    });
    ...
    const handleGroupNotification = (participant, child, msg) => {
        var _a, _b, _c, _d;
        const key = `${child.tag}:${participant}`;
        if (groupNotificationSpamDB.get(key)) {
            return;
        }
        groupNotificationSpamDB.set(key, true);
    ...
    const handleCall = async (node) => {
        ...
        if (callSpamDB.get(from)) {
            logger.debug({ from, callId }, 'ignored call spam');
            await sendMessageAck(node);
            return;
        }
        callSpamDB.set(from, true);
